<?php
    ...
    if(!isset($_SESSION['loged'])){
      ...
      if($_SESSION['aux']==0){   
        ...
        if($conexion){
            $procedimiento = 'DECLARE BEGIN trabajo.GENERAR_COMPRA_DOS(:ID,:ID_BOLETA); END;';
    ...
    $c = oci_connect('trabajo_cliente','trabajo_cliente','localhost/XE');
          if($c){
            $selectt = 'SELECT * FROM trabajo.BOLETA WHERE CLIENTE = :p ORDER BY FECHA, HORA';
            $stmtt = oci_parse($c, $selectt);
            oci_bind_by_name($stmtt, ":p", $_SESSION["rut"]);
            oci_execute($stmtt);
            oci_fetch_all($stmtt, $salidaa);
            $id = $salidaa["ID_BOLETA"][count($salidaa["ID_BOLETA"])-1];
            $fecha = $salidaa["FECHA"][count($salidaa["ID_BOLETA"])-1];
...
require_once('cliente_boleta.php');
...
$subtotal = 0;
$multa= (int)$salidaa["MULTA"][count($salidaa["ID_BOLETA"])-1];
$final= (int)$salidaa["TOTAL_FINAL"][count($salidaa["ID_BOLETA"])-1];
...
$mpdf->writeHtml('</tbody>
    ...
    for ($j=0; $j < $tamano ; $j++){
      ...
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">Id del pasaje:  ' .$_SESSION['datos_pasajes_cliente'][$j][0].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">Nombre del pasajero:  ' .$_SESSION['datos_pasajes_cliente'][$j][1].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">N° de documento:  ' .$_SESSION['datos_pasajes_cliente'][$j][2].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">Asiento:   ' .$_SESSION['datos_pasajes_cliente'][$j][3].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">Origen:   ' .$_SESSION['datos_pasajes_cliente'][$j][4].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">Destino:   ' .$_SESSION['datos_pasajes_cliente'][$j][5].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">Fecha de salida:   ' .$_SESSION['datos_pasajes_cliente'][$j][6].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">Fecha de llegada:   ' .$_SESSION['datos_pasajes_cliente'][$j][7].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">N° de vuelo:   ' .$_SESSION['datos_pasajes_cliente'][$j][8].'
                    </h4>');
      $mpdf->writeHtml('<h4 style="margin-top: 5px;margin-left: 9px;color: #0D5853;transform: scale(1.1);">Tarifa:   ' .$_SESSION['datos_pasajes_cliente'][$j][9].'
                    </h4> <hr>');
