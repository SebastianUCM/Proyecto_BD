...
DROP SEQUENCE ID_BOLETA;
DROP SEQUENCE ID_DESTINO;
DROP SEQUENCE ID_ITINERARIO;
DROP SEQUENCE ID_ORIGEN;
DROP SEQUENCE ID_PASAJE;
...
-- CREACION DE FUNCION PARA VERIFICAR LA EXISTENCIA DE UN ASIENTO

CREATE OR REPLACE FUNCTION VERIFICAR_ASIENTO(
	ID_A IN ASIENTO.ID_ASIENTO%TYPE
) RETURN BOOLEAN
IS 
	AUX NUMBER;
BEGIN 
	SELECT COUNT(*) INTO AUX FROM ASIENTO WHERE ID_ASIENTO=ID_A;
	IF AUX>0 THEN 
		RETURN TRUE;
	ELSE 
		RETURN FALSE;
...
END;
/
CREATE OR REPLACE FUNCTION VERIFICAR_VUELO_ASIENTO(
	ID_A IN VUELO_ASIENTO.ID_ASIENTO%TYPE,
	ID_V IN VUELO_ASIENTO.ID_VUELO%TYPE
) RETURN BOOLEAN
IS 
	AUX NUMBER;
BEGIN 
	SELECT COUNT(*) INTO AUX FROM VUELO_ASIENTO WHERE ID_ASIENTO=ID_A AND ID_VUELO=ID_V;
	IF AUX>0 THEN 
		RETURN TRUE;
	ELSE 
		RETURN FALSE;
...
END;
/
-- CREACION DE PROCEDIMIENTO PARA BORRAR DATOS A LA TABLA ASIENTO

CREATE OR REPLACE PROCEDURE BORRAR_VUELO_ASIENTO
(
	 ID_A  IN VUELO_ASIENTO.ID_ASIENTO%TYPE,
	 ID_V IN VUELO_ASIENTO.ID_VUELO%TYPE
)
IS 
    VUELO_ASIENTO_NO_EXISTE EXCEPTION;
BEGIN 
	IF VERIFICAR_VUELO_ASIENTO(ID_A,ID_V)= TRUE THEN 
		LOCK TABLE VUELO_ASIENTO IN ROW EXCLUSIVE MODE;
		DELETE FROM VUELO_ASIENTO WHERE ID_ASIENTO=ID_A AND ID_VUELO=ID_V;
		COMMIT;
	ELSE
		RAISE VUELO_ASIENTO_NO_EXISTE;
	END IF;

	EXCEPTION
        WHEN VUELO_ASIENTO_NO_EXISTE THEN 
			RAISE_APPLICATION_ERROR(-20001,'ASIENTO DE VUELO INGRESADO NO EXISTE');
			ROLLBACK;
...
END;
/
-- CREACION DE PROCEDIMIENTO PARA ACTUALIZAR DATOS A LA TABLA ASIENTO

CREATE OR REPLACE PROCEDURE ACTUALIZAR_VUELO_ASIENTO
(
	 ID_A  IN VUELO_ASIENTO.ID_ASIENTO%TYPE,
	 ID_V IN VUELO_ASIENTO.ID_VUELO%TYPE,
	 EST IN VUELO_ASIENTO.ESTADO%TYPE
)
IS 
    VUELO_ASIENTO_NO_EXISTE EXCEPTION;
	ESTADO_INCORRECTO EXCEPTION;

BEGIN 
	IF VERIFICAR_VUELO_ASIENTO(ID_A,ID_V)= TRUE THEN 
			IF UPPER(EST) IN ('DISPONIBLE','NO DISPONIBLE') THEN
				LOCK TABLE VUELO_ASIENTO IN ROW EXCLUSIVE MODE;
		        UPDATE VUELO_ASIENTO
		        SET ESTADO=EST
		        WHERE ID_ASIENTO= ID_A AND ID_VUELO= ID_V;
				COMMIT;
			ELSE
				RAISE ESTADO_INCORRECTO;
			END IF;
	ELSE
		RAISE VUELO_ASIENTO_NO_EXISTE;
	END IF;
	...
	EXCEPTION
        WHEN VUELO_ASIENTO_NO_EXISTE THEN 
			RAISE_APPLICATION_ERROR(-20001,'ASIENTO A ACTUALIZAR NO EXISTE');
			ROLLBACK;

		WHEN ESTADO_INCORRECTO THEN 
...
BEGIN
	...
	EDAD := 2020 - TO_NUMBER(SUBSTR(FECHA,7,4));
    IF EDAD > 17 THEN
        RETURN TRUE;
    ELSE
        RETURN FALSE;
...
BEGIN
				...
				IF VERIFICAR_EDAD(NUEVA_FECHA) = TRUE THEN
					IF LENGTH(TELEF) IN (9,10,11,12) THEN
						IF LENGTH(CONTRA) >7 AND LENGTH(CONTRA) <21 THEN
							...
							COMMIT;
							RESULTADO := 'TRUE';
							MENSAJE :='USTED FUE REGISTRADO CON EXITO';
						ELSE
	...
	END IF;
   	...
   	WHEN EMAIL_EXISTE THEN
            RESULTADO := 'FALSE';
        	MENSAJE :='EL CORREO ELECTRONICO INGRESADO YA SE ENCUENTRS VINCULADO A UNA CUENTA';
            ROLLBACK;
    WHEN DOCUMENTO_INCORRECTO THEN
            RESULTADO := 'FALSE';
        	MENSAJE :='EL NUMERO DE DOCUMENTO NO EXISTE O ES INCORRECTO';
            ROLLBACK;
	...
	WHEN CONTRASENA_INSEGURA THEN
            RESULTADO := 'FALSE';
        	MENSAJE :='LA CONTRASEÑA ES INSEGURA, DEBE CONTENER 8 DIGITOS';
            ROLLBACK;
...
---------------------------  PAGO ----------------------------------
----------------------------------------------------------------------
-- PARA OBTENER EL VALOR A PAGAR EN VISTA PAGO SIN INSERTAR AUN BOLETA DETALLE
CREATE OR REPLACE PROCEDURE OBTENER_PAGO_TOTAL(
    ID IN CLIENTE.NUMERO_DOCUMENTO%TYPE,
    VALOR OUT NUMBER
)
IS
...
DECLARE
    ...
    MENSAJE NVARCHAR2(100);
BEGIN
    INSERTAR_VUELO('AVION-1','ITNR-2',8000, resultado, mensaje);
    DBMS_OUTPUT.PUT_LINE('RESULTADO: '||RESULTADO);
    DBMS_OUTPUT.PUT_LINE('MENSAJE: '||MENSAJE);
END;
/
DECLARE
    ...
    MENSAJE NVARCHAR2(100);
BEGIN
    INSERTAR_VUELO('AVION-1','ITNR-3',8000, resultado, mensaje);
    DBMS_OUTPUT.PUT_LINE('RESULTADO: '||RESULTADO);
    DBMS_OUTPUT.PUT_LINE('MENSAJE: '||MENSAJE);
END;
/
DECLARE
    ...
    MENSAJE NVARCHAR2(100);
BEGIN
    INSERTAR_VUELO('AVION-1','ITNR-4',8000, resultado, mensaje);
    DBMS_OUTPUT.PUT_LINE('RESULTADO: '||RESULTADO);
    DBMS_OUTPUT.PUT_LINE('MENSAJE: '||MENSAJE);
END;
/
DECLARE
    ...
    MENSAJE NVARCHAR2(100);
BEGIN
    INSERTAR_VUELO('AVION-2','ITNR-5',50000, resultado, mensaje);
    DBMS_OUTPUT.PUT_LINE('RESULTADO: '||RESULTADO);
    DBMS_OUTPUT.PUT_LINE('MENSAJE: '||MENSAJE);
END;
/
DECLARE
    ...
    MENSAJE NVARCHAR2(100);
BEGIN
    INSERTAR_VUELO('AVION-1','ITNR-6',9000, resultado, mensaje);
    DBMS_OUTPUT.PUT_LINE('RESULTADO: '||RESULTADO);
    DBMS_OUTPUT.PUT_LINE('MENSAJE: '||MENSAJE);
END;
/
