...
$num=count($asientos); 
            ...
            $datos_pasajes_cliente = array();
                for ($j=1; $j <=$num; $j++){
                    $aux_pasajes = array();
                    array_push($aux_pasajes,$resultados['ID_PASAJE'][$j-1]);
                    array_push($aux_pasajes,$resultados['NOMBRE_COMPLETO'][$j-1]);
                    array_push($aux_pasajes,$resultados['PASAJERO'][$j-1]);
                    array_push($aux_pasajes,$resultados['ASIENTO'][$j-1]);
                    array_push($aux_pasajes,$resultadosxx['ORIGENN'][$j-1]);
                    array_push($aux_pasajes,$resultadosx['DESTINOO'][$j-1]);
                    array_push($aux_pasajes,$resultadosx['SALIDA'][$j-1]);
                    array_push($aux_pasajes,$resultadosx['LLEGADA'][$j-1]);
                    array_push($aux_pasajes,$resultados['VUELO'][$j-1]);
                    array_push($aux_pasajes,$resultadost['NOMBRE'][$j-1]);
                    array_push($datos_pasajes_cliente,$aux_pasajes);
                    unset($aux_pasajes);
                ...
                $_SESSION['aux']=0;
                $_SESSION['datos_pasajes_cliente'] = $datos_pasajes_cliente;
                                           
            }?>
            <div class="container  mt-5">
            <button type="submit" class="btn btn-success float-right" onclick="location.href='cliente_pago.php'" style="width:200px;background-color: #041673;margin-right:80px;">Continuar</button><br>
            </div><br><br>