<?php
    session_start();
      ...
      if($_SESSION['aux']==0){   
        ...
        $conexion = oci_connect('trabajo','trabajo','localhost/XE');
        if($conexion){
            $procedimiento = 'DECLARE BEGIN GENERAR_COMPRA(:ID); END;';
    ...
    $c = oci_connect('trabajo','trabajo','localhost/XE');
          if($c){
            $select = 'SELECT * FROM BOLETA WHERE CLIENTE = :p ORDER BY FECHA, HORA';
            ...
            $select = 'SELECT * FROM DETALLE WHERE ID_BOLETA = :p';
            $stmt = oci_parse($c, $select);
...
$mpdf->WriteHTML('<header class="clearfix">
      <div id="logo">
        <img src="cami.jpg" width="100" height="100">
      </div>
    ...
    <main>
      <div id="details" class="clearfix">
        <div id="client">
          <div class="to">SKY:</div>
          <div class="email"><a href="sky.vuela@ksy.cl">sky.vuela@ksy.cl</a></div>
...
$mpdf->writeHtml('</tbody>
      ...
      <div id="notices">
        <div>IMPORTANTE:</div>
        <div class="notice">Puedes cambiar itinerario, la cual se te cobrará una multa de 20% al total de tu viaje.</div>
      </div>
    </main>
    <footer>
...
for ($j=0; $j < $tamano ; $j++){
  $mpdf->writeHtml('<div id="logo">
        <img src="aeroskiwhite.png" width="500" height="500" right= "900">
      </div>'); 
